from admin import models, faforms
import logging
...
def includeme(config):
    ...
    if session_factory is not None:
        # pyramid_alchemy
        session_factory = 'admin.models.DBSession'
    else:
        # Akhet
        session_factory = 'admin.models.Session'

    # register session and model_view for later use
    settings = {'package': 'admin',
                'view': model_view,
                'session_factory': session_factory,
               }
    config.registry.settings['admin.fa_config'] = settings

    config.formalchemy_admin("/admin", models=models, forms=faforms,
                             **settings)

    # Adding the package specific routes
    config.include('admin.faroutes')
